...
from nucleus.utils.inf_stabilizer import clip_temp_by_phase, NormalizedTempLimits
...
class Nucleus2MoEDivFree(nn.Module):    
    ...
    def forward_trajectory(
        ...
        normalized_sim_params: torch.Tensor,
        dx: float,
        ...
        assert normalized_sim_params.dim() == 2, "fluid params must be [B, num_params]"
        assert initial_state.shape[0] == normalized_sim_params.shape[0]
        ...
        for _ in range(input_time_window_size, trajectory_steps, output_time_window_size):
            pred, moe_outputs = self.step(trajectory[:, -input_time_window_size:], normalized_sim_params)
